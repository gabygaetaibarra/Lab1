...
(() => {
  ...
  // Entities
  const player= {x:BASE_WIDTH/2, y:BASE_HEIGHT-140, r:18, angle:-Math.PI/2, speed:240, vx:0, vy:0};
  const bullets = [];
  const enemies = [];
  const particles = [];
  // Background image
  ...
  function render() {
    ...
    for (const e of enemies) {
      ...
      ctx.drawImage(enemyImage, -e.r, -e.r, e.r * 2, e.r * 2);
      ctx.restore();
    }
    ctx.restore();
    // Bullets
    ...
    ctx.drawImage(playerImage, -player.r, -player.r, player.r * 2, player.r * 2);
    ctx.restore();
    // Particles
    ctx.save();
